...
PKG_PREFIX=$(ros2 pkg prefix serial_link_impedance_control)
CONFIG_DIR="${PKG_PREFIX}/share/serial_link_impedance_control/config"

# Open a new terminal and run the action server
gnome-terminal -- bash -c "ros2 launch serial_link_impedance_control trajectory_tracking.py model:=${MODEL}; exec bash"
